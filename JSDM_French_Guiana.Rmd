...
df2 <- dplyr::left_join(df2, coord_df2, by=c("Plot", "Forest"))
...
# New column forest_plot (same IdPlot for different forest between projects)
...
colnames(coords) <- c("Xutm","Yutm","plot", "sourceCoord", "UTMZone", "Project")
coords_Guyafor <- coords[coords$Project=="Guyafor",]
coords_Guyafor_plot <- coords_Guyafor[coords_Guyafor$sourceCoord=="plot",]
coords_Guyafor_tree <- coords_Guyafor[coords_Guyafor$sourceCoord=="tree",]
coords_Guyafor_tree <- coords_Guyafor_tree[!(coords_Guyafor_tree$plot %in% coords_Guyafor_plot$plot), ]
sites <- unique(coords_Guyafor_tree[,"plot"])
coords_Guyafor <- coords_Guyafor_plot 
for(i in 1:length(sites)){
  mat <- coords_Guyafor_tree[coords_Guyafor_tree$plot==sites[i], ]
  coords_Guyafor <- rbind(coords_Guyafor, mat[which.min(mat$Xutm),])
}
