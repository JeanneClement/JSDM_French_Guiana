...
df <- dplyr::left_join(df, habitat_plots)
...
df2  <- select(df2, -c("Xutm", "Yutm", "Lat", "Lon"))
...
coord_df2 <- dplyr::select(coord_df2, c("Forest", "Plot", "XutmSO", "YutmSO", "LatSO", "LonSO", "UTMZone"))
names(coord_df2) <- c("Forest", "Plot", "Xutm", "Yutm", "Lat", "Lon", "UTMZone")
df2 <- dplyr::left_join(df2, coord_df2, relationship = "many-to-many",
                        by=c("Plot", "Forest"))
...
trees[, c("Lat", "Lon", "PlotArea", "Topo", "SourceCoord")] <- NA
trees[trees$Project=="GuyaDiv", c("Lat", "Lon", "PlotArea")] <- df2[, c("Lat", "Lon", "PlotArea")]
trees[trees$Project=="GuyaDiv", c("SourceCoord")] <- "plot"
trees[!(trees$Project=="GuyaDiv"), c("Topo", "SourceCoord")] <- df[, c("Topo", "SourceCoord")]
...
n_common_plot <- sum(unique(trees$idPlot[trees$Project=="GuyaDiv"]) %in% unique(trees$idPlot[!(trees$Project=="GuyaDiv")]))
species <- unique(trees$Taxon)
nsp <- length(species)
n_common_sp <- sum(unique(df2$Taxon) %in% unique(df$Taxon))
# presence/absence of each species on each plot
PA <- matrix(0, nplot, nsp)
rownames(PA) <- unique(trees$idPlot)
colnames(PA) <- unique(trees$Taxon)
for (i in 1:nplot){    
  for (j in 1:nsp){
    idx <- which(trees$Taxon == species[j])
    PA[unique(trees$idPlot[idx]),j] <- 1 
  }
}
...
knitr::kable(head(PA_anon[,1:20]), digits=1, booktabs=TRUE, align = 'c') %>%
...
\cr 
\cr
```{r FG-PA, echo=FALSE, out.width=900, eval=FALSE}
...
coords <- unique(dplyr::select(trees, c(Xutm, Yutm,
                                        idPlot, SourceCoord, UTMZone, Project)))
# Project Lat Lon coordinates to UTM22N coordinates (EPSG:32622)
# terra::project(terra::vect(matrix(c(-54.4, 2.21), ncol=2), type="point", crs="epsg:4326"), "epsg:32621")
# UTM 21N EPSG:32621

# Vector of coordinates for each inventory site
plots <- unique(coords$idPlot)
nplot <- length(plots)
xy <- matrix(NA, nrow=nplot, ncol=3)
colnames(xy) <- c("x", "y", "EPSG")
rownames(xy) <- plots 
coords_plot <- coords[coords$SourceCoord=="plot",]

for(i in 1:nplot){
  
  if(!("plot" %in% coords$SourceCoord[coords$idPlot == plots[i]])){
    xy[i, c("x","y")] <- apply(as.matrix(coords[coords$idPlot == plots[i] ,
                                                c("Xutm", "Yutm")]), 2, min, na.rm=TRUE)
  }
  
  else{
    xy[i, c("x","y")] <- unique(as.matrix(coords_plot[coords_plot$idPlot == plots[i], c("Xutm", "Yutm")]))
    
  }  
  xy[i,"EPSG"] <- paste0("326", unique(as.matrix( 
    coords$UTMZone[coords$idPlot == plots[i]])))
}
...
# terra::writeVector(xy, overwrite=TRUE, "~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/xy.shp")
