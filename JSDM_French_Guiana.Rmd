...
```{r load-librairies, echo=FALSE, eval=TRUE}
library(terra)
library(sf)
library(jSDM)
library(gecevar)
...
###  Annual Plant Available Water Deficit (APAWD)
...
Forest inventories carried out in French Guiana, in the context of Guyafor, Guyadiv, Habitat,  Diadema, AmaLin and Paracou project are available. However, the inventory sites considered in the different project haven't differ in shape and size.  Consequently, inventory sites were redefined as 1km cells. We use these forest inventories to calculate a matrix indicating the presence by a $1$ and the absence by a $0$ of the species at each inventory site by removing observations for which the species is not identified and rare species with less than 2 observations on all sites. This matrix therefore records the occurrences of $1246$ species at $544$ sites.
...
### Define new inventory plots as 1km cells 
...
write.csv(PA_anon, file ="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/PA_anon.csv")
knitr::kable(PA_anon[1:20,1:20], digits=1, booktabs=TRUE, align = 'c') %>%
  kableExtra::kable_styling(latex_options=c("HOLD_position","striped"), full_width=FALSE) %>% 
...
# get interesting covariates among the climatic variables downloaded  
clim_var <- terra::rast(clim_path,
                        lyrs=c("bio1", "bio12", "bio15", "cwd_penman"))
names(clim_var) <- c("temp", "prec", "sais_prec", "cwd")
proj <- terra::crs(clim_var)
...
We can see that temperature seasonality is positively correlated with the precipitation and the annual plant available water deficit, so we won't take this variable into account when fitting JSDMs on French Guiana. As expected  the Climatic Water Deficit (CWD) is strongly and positively correlated with the annual plant available water deficit, so we won't consider the CWD for fitting JSDMs. We prefer to keep the APAWD, which takes into account soil variables and field capacity in addition to potential evapotranspiration (quote-Salman) and precipitation, unlike the CWD. 

# Design matrix 

We also consider the quadratic effects of these climate variables to perform a quadratic regression, which is more suitable for fitting a niche model than a linear regression.

```{r design-matrix, echo=FALSE, eval=FALSE}
...
colnames(plots_coords) <- c("idPlot", "Lon", "Lat")
coords <- terra::vect(plots_coords, geom=c("Lon", "Lat"), crs="epsg:4326")
remove(trees)
# Climatic variables 
clim_path <- "~/Documents/projet_METRADICA/data/data_raw/current_chelsa.tif"
load("~/Documents/projet_METRADICA/data/output.RData")
...
clim_var <- c(APAWD, clim_var)

# Project to latlon coordinates system 
clim_var <- terra::project(clim_var, "epsg:4326")
...
# Remove sites with missing climatic variables values
# (check plots coordinates)
clim2 <- stats::na.omit(clim2)
plots_outside <- as.numeric(attr(clim2, "na.action"))
plots_coords <- plots_coords[-plots_outside,]
coords <- terra::vect(plots_coords, geom=c("Lon", "Lat"), crs="epsg:4326")
...
nplot <- nrow(plots_coords)
PA <- readr::read_delim("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/PA_anon.csv", delim=",")
PA <- PA[-plots_outside,]
write.csv(PA, file ="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/PA_anon.csv")
# Add squared data
data_clim2 <- data.frame(cbind(clim2[,-1], plots_coords))
...
scaled_data_clim2 <- as_tibble(cbind(plot=data_clim2$idPlot,
                                     scaled_data_clim2,
                                     Lon=data_clim2$Lon, 
                                     Lat=data_clim2$Lat))
write.csv(scaled_data_clim2[,-1], file ="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/scaled_clim.csv")

## Design matrix
X <- data.frame(intercept=rep(1, nplot),
                dplyr::select(scaled_data_clim2,-Lon,-Lat, -plot))
np <- ncol(X)
...
scaled_data_clim2 <- read_csv("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/scaled_clim.csv")
kable(head(scaled_data_clim2[,-1]), digits=3) %>%
...
We fit four binomial joint species distribution models, considering different combination of explanatory variables.
We use the `jSDM_binomial_probit()` function to perform binomial probit regressions considering the presence-absence matrix of species on inventory sites, described above, by performing $30 000$ iterations including $15 000$ of burn-in and we retain $N_{samp}=1 000$ values for each parameter of the models. 

```{r select-JSDM-FG, echo=TRUE, eval=FALSE} 
# presence-absence matrix
PA <- readr::read_delim("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/PA_anon.csv", delim=",")
PA <- as.data.frame(PA[,-1])
rownames(PA) <- PA[,1]
PA <- PA[,-1]
# Design matrix 
scaled_data_clim2 <- readr::read_csv("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/scaled_clim.csv")
# Fitting JSDMs from French Guiana dataset
# M0 : Intercept only
T1<- Sys.time()
mod0 <- jSDM::jSDM_binomial_probit(
  # Response variable 
  presence_data = PA, 
  # Explanatory variables 
  site_formula = ~ Int -1,   
  site_data=data.frame(Int=rep(1, nrow(scaled_data_clim2))), 
  n_latent=0,
  site_effect="none",
  # Chains
  burnin= 15000, mcmc=15000, thin=15,
  # Starting values
  beta_start=0,
  # Priors
  mu_beta=0, V_beta=1,
...
T_0 <- difftime(T2,T1)
save(mod0, T_0,
     file="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/mod0_FG.RData")

# M1 : X=c(1, apawd, apawd2) 
T1<- Sys.time()
mod1 <- jSDM::jSDM_binomial_probit(
  # Response variable 
  presence_data = PA, 
  # Explanatory variables 
  site_formula = ~ apawd + apawd2,   
...
T_1 <- difftime(T2,T1)
save(mod1, T_1,
     file="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/mod1_FG.RData")

# M2 : X=c(1, cwd, cwd2) 
T1<- Sys.time()
mod2 <- jSDM::jSDM_binomial_probit(
  # Response variable 
  presence_data = PA, 
  # Explanatory variables 
  site_formula = ~ cwd + cwd2,   
  site_data=scaled_data_clim2, 
  n_latent=0,
  site_effect="none", 
  # Chains
...
T_2 <- difftime(T2,T1)
save(mod2, T_2,
     file="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/mod2_FG.RData")

 
# M3:  
# X=c(1, temp, prec, sais_prec, apawd, temp2, prec2, sais_prec2, apawd2)
T1<- Sys.time()
mod3 <- jSDM::jSDM_binomial_probit(
  # Response variable 
  presence_data = PA, 
  # Explanatory variables 
  site_formula = ~ temp + prec + sais_prec + apawd + temp2 + prec2 +  sais_prec2 + apawd2,   
  site_data=scaled_data_clim2, 
  n_latent=0,
  site_effect="none", 
  # Chains
...
T_3 <- difftime(T2,T1)
save(mod3, T_3,
     file="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/mod3_FG.RData")
# M3:  
# X=c(1, temp, prec, sais_prec, apawd, temp2, prec2, sais_prec2, apawd2)
T1<- Sys.time()
mod4 <- jSDM::jSDM_binomial_probit(
  # Response variable 
  presence_data = PA, 
  # Explanatory variables 
  site_formula = ~ temp + prec + sais_prec + cwd + temp2 + prec2 +  sais_prec2 + cwd2,   
  site_data=scaled_data_clim2, 
  n_latent=0,
  site_effect="none", 
  # Chains
...
T_4 <- difftime(T2,T1)
save(mod4, T_4,
     file="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/mod4_FG.RData")
```

```{r fit-JSDM-FG-comp-table, echo=FALSE, eval=FALSE}
for(i in 0:4){
load(paste0("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/mod", i, "_FG.RData"))
}
nsp <- ncol(mod0$model_spec$presence_data)
nplot <- nrow(mod0$model_spec$presence_data)
results <- data.frame(nsp, nplot,
                      n_obs=nsp*nplot,  n_param=NA, p=NA,
                      variables=c("None", "apawd", "cwd",
                                  "temp, prec, sais_prec, apawd",
                                  "temp, prec, sais_prec, cwd"),
                      ngibbs="30000", Time = NA, dev=NA, chi2=NA)
for(i in 0:4){
  mod <- get(paste0("mod", i))
  p <- nrow(mod$model_spec$beta_start)
  results[i+1, "p"] <- p
  results[i+1, "n_param"] <- p*nsp 
  results[i+1, "dev"] <- mean(mod$mcmc.Deviance)
  results[i+1, "Time"] <- as.numeric(get(paste0("T_", i)), units="secs")
  # 100 * (Null deviance – Model's deviance)/Null deviance
    results[i+1, "chi2"] <- ((results[1, "dev"] - mean(mod$mcmc.Deviance))/results[1, "dev"])*100
}

colnames(results) <- c("Species", "Sites",
                       "Observations",
                       ...
                       "Deviance",
                       "Deviance explained (%)")
rownames(results) <- c("Model null", "Model 1",
                       "Model 2", "Model 3", "Model 4")

knitr::kable(results, row.names=TRUE, digits=1, booktabs=TRUE,
             escape=FALSE, align = 'l') %>%
  kableExtra::kable_styling(latex_options=c("HOLD_position","striped"), full_width=FALSE) %>% 
  kableExtra::save_kable(file="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/comp_mod_FG.png")
```

\cr 
\cr
```{r FG-fitting-mod, echo=FALSE, out.width=1000, eval=TRUE}
knitr::include_graphics("JSDM_French_Guiana_files/comp_mod_FG.png")
```



### Fitting the JSDM selected 

We fit the binomial joint species distribution model selected above, including **two latent variables** and a **random site effect**.
We use the `jSDM_binomial_probit()` function to perform binomial probit regressions considering the presence-absence matrix of species on inventory sites, described above, by performing $30 000$ iterations including $15 000$ of burn-in and we retain $N_{samp}=1 000$ values for each parameter of the model. 

```{r fit-jSDM-FG, echo=TRUE, eval=FALSE} 
# presence-absence matrix
PA <- readr::read_delim("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/PA_anon.csv", delim=",")
PA <- as.data.frame(PA[,-1])
rownames(PA) <- PA[,1]
...
# Design matrix 
scaled_data_clim2 <- readr::read_csv("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/scaled_clim.csv")
# Fitting the JSDM selected from French Guiana dataset

# Intercept only (X=1)
T1<- Sys.time()
modnull <- jSDM::jSDM_binomial_probit(
  # Response variable 
  presence_data = PA, 
  # Explanatory variables 
  site_formula = ~ Int - 1,    
  site_data=data.frame(Int=rep(1,nplot)), 
  n_latent=2,
  site_effect="random", 
  # Chains
  burnin= 15000, mcmc=15000, thin=15,
  ...
  seed=1234, verbose=1)
T2 <- Sys.time()
T_null <- difftime(T2,T1)
save(modnull, T_null,
     file="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/modnull_FG.RData")
...
load("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/modnull_FG.RData")
...
results <- data.frame(nsp, nplot,
                      n_obs=nsp*nplot,  n_latent=mod$model_spec$n_latent,
                      n_param=NA, p=NA,
                      variables=c("None",  "temp, prec, sais_prec, apawd"),
                      ngibbs="30000", Time = NA, dev=NA, chi2=NA)
results[1, "p"] <- nrow(modnull$model_spec$beta_start)
results[2, "p"] <- nrow(mod$model_spec$beta_start)
results[1, "n_param"] <- nsp + modnull$model_spec$n_latent*nsp -1 + nplot + 1 +
  modnull$model_spec$n_latent*nplot 
results[2, "n_param"] <-  nrow(mod$model_spec$beta_start)*nsp + 
  mod$model_spec$n_latent*nsp -1 + nplot + 1 + 
  mod$model_spec$n_latent*nplot 
results[1, "dev"] <- mean(modnull$mcmc.Deviance)
results[2, "dev"] <- mean(mod$mcmc.Deviance)
results[1, "Time"] <- as.numeric(T_null, units="secs")
results[2, "Time"] <- as.numeric(T_all, units="secs")
# 100 * (Null deviance – Model's deviance)/Null deviance
results[2, "chi2"] <- ((results[1, "dev"] - mean(mod$mcmc.Deviance))/results[1, "dev"])*100
...
colnames(results) <- c("Species", "Sites",
                       ...
                       "Deviance",
                       "Deviance explained (%)")
rownames(results) <- c("Model null", "Model")
...
\cr 
\cr
```{r FG-table, echo=FALSE, out.width=1000, eval=TRUE}
...
params_sites <- data.frame(plot = 1:nrow(scaled_data_clim2),
                           Lon = scaled_data_clim2$Lon,
                           Lat = scaled_data_clim2$Lat,
                           alphas, V_alpha = rep(V_alpha,nplot),W1,W2)
write.csv2(params_sites, "~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/params_sites.csv")
...
write.csv2(params_species, "~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/params_species.csv")
```
...
PA <- readr::read_delim("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_files/PA_anon.csv", delim=",")
load("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/mod_FG.RData")
params_sites <-  read.csv2("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/params_sites.csv")
PA <- as.data.frame(PA[,-1])
PA <- PA[,-1]
...
id_pres <- which(PA[, which.max(colSums(PA))]==1)
id_abs <- which(PA[, which.max(colSums(PA))]==0)
obs_pres <-  terra::vect(params_sites[id_pres, c("Lat", "Lon")],
                         geom=c("Lon", "Lat"), 
                         crs="epsg:4326")
obs_abs <-  terra::vect(params_sites[id_abs, c("Lat", "Lon")],
                        geom=c("Lon", "Lat"),
                        crs="epsg:4326")
# Representation
par(mfrow=c(1,1))
terra::plot(FG_borders, col="lightgray",
            main=paste0("Observed occurences of sp_", which.max(colSums(PA))),
            cex.main=1.4)
...
coords <- terra::vect("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/coords.shp")
theta_latent_sp <- terra::vect(data.frame(terra::crds(coords), mod$theta_latent),
                               crs=terra::crs(coords), geom=c("x","y"))
# Representation
...
terra::plot(FG_borders, col="lightgray",
            main =paste0("Estimated probabilities of presence for sp_",
                         which.max(colSums(PA))),
            cex.main=1.4)
...
species_richness_obs_sp <- terra::vect(cbind(terra::crds(coords),
                                             species_richness_obs),
                                       geom=c("x","y"), crs=terra::crs(coords))
par(mfrow=c(1,1))
...
species_richness_sp <- terra::vect(cbind(terra::crds(coords),
                                         species_richness), 
                                   geom=c("x","y"), crs=terra::crs(coords))
terra::plot(FG_borders, col="lightgray",
...
### Estimated using JSDM vs SDM 

We compare the species richness obtained from the species' occurrence probabilities fitted by a SDM and a JSDM considering the same explanatory variables. 

```{r species-richness-JSDM-SDM-representation}
...
terra::plot(species_richness_obs_sp, 'species_richness_obs',
            ...
            pch=20, cex=1.2, add=TRUE, 
            legend="topright", plg=list(cex=1.01), type="interval")
# Representation of estimated species richness using SDM
load("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/mod3_FG.RData")
par(mfrow=c(1,1))
species_richness_mod3 <- data.frame(species_richness= apply(mod3$theta_latent,1,sum))
species_richness_sp <- terra::vect(cbind(terra::crds(coords),
                                         species_richness_mod3), 
                                   geom=c("x","y"), crs=terra::crs(coords))
terra::plot(FG_borders, col="lightgray",
            main ="Estimated species richness using SDM",
            cex.main=1.4)
terra::plot(species_richness_sp,
            'species_richness', col=col,
            breaks=cuts, pch=20, cex=1.2, add=TRUE, 
...
# Comparison between observed and estimated species richness at inventory sites
par(mfrow=c(1,1), mar=c(5, 4, 4, 2))
plot(species_richness_mod3$species_richness,
     species_richness$species_richness,
     main="Species richness", xlab="Estimated using SDM",
     ylab="Estimated using JSDM",
     cex.main=1.4)
abline(a=0,b=1,col='red')
```
               
```{r species-richness-JSDM-SDM-representation2, echo=FALSE, fig.show = "hold", out.width = "50%", fig.align = "default", eval=TRUE}
               knitr::include_graphics(c("JSDM_French_Guiana_files/figure-html/species-richness-JSDM-SDM-representation-2.png","JSDM_French_Guiana_files/figure-html/species-richness-JSDM-SDM-representation-1.png"))
```
               
```{r species-richness-JSDM-SDM-representation3, echo=FALSE, fig.show = "hold", out.width = "50%", fig.align = "center", eval=TRUE}
               knitr::include_graphics("JSDM_French_Guiana_files/figure-html/species-richness-JSDM-SDM-representation-3.png")
```
...
Thus, it is possible to interpolate sites' parameters for the whole area from those estimated for the inventory plots using the spatial interpolation method called regularized spline with tension (**RST**) from [`GRASS GIS`](https://grass.osgeo.org/) software via `rgrass7` R package.
...
system('grass -c "~/Documents/projet_METRADICA/data/data_raw/current_chelsa.tif" grassdata/interpolation')
...
# alpha 
alpha_sp <- terra::vect(data.frame(alpha=params_sites$alphas, terra::crds(coords)),
                        geom=c("x","y"), crs=terra::crs(coords))
clim <- terra::rast("~/Documents/projet_METRADICA/data/data_raw/current_chelsa.tif", lyrs=1)
# Project in epsg:32622
alpha_sp <- terra::project(alpha_sp,  clim)
rgrass::write_VECT(alpha_sp, "alpha", flags=c("o", "overwrite"))
# Re-sample with RST
...
system("r.out.gdal --overwrite input=alpha_rst \\
			 output=JSDM_French_Guiana_cache/alpha_rst_m.tif type=Float32 \\
			 createopt='compress=lzw,predictor=2'")
# Representation
alpha_rst_m <- terra::rast("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/alpha_rst_m.tif")
alpha_rst <- terra::project(alpha_rst_m, terra::crs(coords))
...
alpha_rst_xy <- terra::extract(alpha_rst, coords)[,"alpha_rst"]
...
terra::writeRaster(alpha_rst,
                   "~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/alpha_rst.tif",
                   gdal=c("COMPRESS=LZW", "PREDICTOR=2"), overwrite=TRUE)
...
alpha_rst_centered_xy <- terra::extract(alpha_rst_centered, coords)[,"alpha_rst"]
...
knitr::include_graphics(paste0("JSDM_French_Guiana_files/figure-html/RST_alpha-", 1:4, ".png"))
```
...
```{r RST-W1}
# W1 
W1_sp <- terra::vect(data.frame(W1=params_sites$W1, terra::crds(coords)),
                     geom=c("x","y"), crs=terra::crs(coords))
# Project in epsg:32622
clim <- terra::rast("~/Documents/projet_METRADICA/data/data_raw/current_chelsa.tif", lyrs=1)
W1_sp <- project(W1_sp, clim)
rgrass::write_VECT(W1_sp, "W1", flags="overwrite")
# Re-sample with RST
# for punctual data use function v.surf.rst
...
system("r.out.gdal --overwrite input=W1_rst \\
			 output=JSDM_French_Guiana_cache/W1_rst_m.tif type=Float32 \\
			 createopt='compress=lzw, predictor=2'")
# Representation
W1_rst_m <- terra::rast("JSDM_French_Guiana_cache/W1_rst_m.tif")
W1_rst <- terra::project(W1_rst_m, terra::crs(coords))
terra::writeRaster(W1_rst, "JSDM_French_Guiana_cache/W1_rst.tif",
                   gdal=c("COMPRESS=LZW", "PREDICTOR=2"), overwrite=TRUE)
# Data restricted to French Guiana's borders
borders <- terra::vect(output[[2]], layer="gadm36_GUF_0")
...
W1_rst_xy <- terra::extract(W1_rst, coords)[, "W1_rst"]
...
W1_rst_centered_xy <- terra::extract(W1_rst_centered, coords)[,"W1_rst"]
...
knitr::include_graphics(paste0("JSDM_French_Guiana_files/figure-html/RST-W1-", 1:4, ".png")
```
...
```{r RST-W2}
# W2 
W2_sp <- terra::vect(data.frame(W2=params_sites$W2, terra::crds(coords)),
                     geom=c("x","y"), crs=terra::crs(coords))
# Project in epsg:32622
W2_sp <- project(W2_sp, clim)
rgrass::write_VECT(W2_sp, "W2", flags="overwrite")
# Re-sample with RST
...
system("r.out.gdal --overwrite input=W2_rst \\
			 output=JSDM_French_Guiana_cache/W2_rst_m.tif type=Float32 \\
			 createopt='compress=lzw,predictor=2'")
# Representation
W2_rst_m <- terra::rast("JSDM_French_Guiana_cache/W2_rst_m.tif")
W2_rst <- terra::project(W2_rst_m, terra::crs(coords))
terra::writeRaster(W2_rst, "JSDM_French_Guiana_cache/W2_rst.tif",
                   gdal=c("COMPRESS=LZW", "PREDICTOR=2"), overwrite=TRUE)
# Data restricted to French Guiana's borders
borders <- terra::vect(output[[2]], layer="gadm36_GUF_0")
...
W2_rst_xy <- terra::extract(W2_rst, coords)[, "W2_rst"]
...
W2_rst_centered_xy <- terra::extract(W2_rst_centered, coords)[,"W2_rst"]
...
knitr::include_graphics(paste0("JSDM_French_Guiana_files/figure-html/RST-W2-", 1:4, ".png")
```
...
# Species parameters 
params_species <- read.csv2(file = "~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/params_species.csv", sep=";", header=TRUE)
nsp <- nrow(params_species)
# Design matrix  of current climatic variables 
scaled_clim_var <- terra::rast("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/scaled_clim.tif",
                               lyrs=c("temp", "prec", "sais_prec", "apawd",  "temp2", "prec2", "sais_prec2", "apawd2"))
...
predfun <- function(scaled_clim_var, params_species, rst_alpha, rst_W1, rst_W2, species.range){
  ...
  beta <- as.matrix(params_species[ , grep("beta", colnames(params_species))])
...
```{r plot-theta}
params_species <- read.csv2(file = "~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/params_species.csv",
                            header = TRUE, sep=";")
nsp <- nrow(params_species)
coords <- terra::vect("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/coords.shp")
...
obs_pres<-  terra::vect(terra::crds(coords[id_pres,]),
                                type="point", crs=terra::crs(coords))
id_pres2 <- which(PA[,"sp_6"]==1)
obs_pres2 <-  terra::vect(terra::crds(coords[id_pres2,]),
                                   type="point", crs=terra::crs(coords))
# Representation
npart <- 10
first.species <- seq(1,nsp,by=floor(nsp/npart)+1)
theta <- terra::rast("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/RST_centered_theta_1.tif")
...
terra::plot(theta$sp_1,
            main="sp_1")
terra::points(obs_pres, pch=3, cex=0.5)
terra::plot(theta$sp_6,
            main="sp_6")
terra::points(obs_pres2, pch=3, cex=0.5)
...
npart <- 10
for(n in 2:npart){
  theta <- terra::rast(paste0("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/RST_centered_theta_",
                              n, ".tif"))
...
However, the model used does not take into account human presence, which is manifested in particular by the deforestation of the territory, so we use data on remaining forest cover in 2010 downloaded from <https://forestatrisk.cirad.fr/index.html> computed following the article @Vieilledent2022, using the [get_env_variables](https://ecology.ghislainv.fr/gecevar/reference/get_env_variables.html) function of package [gecevar](https://ecology.ghislainv.fr/gecevar/), in order to replace with zero values the interpolated species richness at locations where forest is known to have been removed. 

```{r species-richness-deforest, eval=FALSE}
library(glue)
# raster of French Guiana's forest cover in 2010
output_file <- "/home/clement/Documents/projet_METRADICA/data"
setwd("~/Documents/projet_METRADICA/data/")
epsg <- 32622
name <- "French Guiana"
load(file="~/Documents/projet_METRADICA/data/output.RData")
extent <- output[[1]][1]
extent_latlon <- as.numeric(output[[1]][2:5])
env.path <- gecevar::get_env_variables(extent_latlon = extent_latlon,
                                       extent = extent,
                                       EPSG = epsg,
                                       country_name = name,
                                       destination = output_file,
                                       ...
                                       rm_download = TRUE,
                                       forest_year = 2010)
forest <- terra::rast(env.path, lyrs=c("forest"))
# change projection of species richness raster from lat lon to epsg 32622
species_richness <- terra::rast("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/species_richness.tif")
species_richness_m <- project(species_richness, forest)
terra::writeRaster(species_richness_m,
                   filename="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/species_richness_m.tif",
                   filetype="GTiff", gdal=c("COMPRESS=LZW", "PREDICTOR=2"), overwrite=TRUE)
# remove species richness values where there was no forest in 2010s
species_richness_deforest <- terra::mask(species_richness_m, forest)
...
# raster of French Guiana's forest cover in 2010
env.path <- "~/Documents/projet_METRADICA/data/data_raw/environ.tif"
forest <- terra::rast(env.path, lyrs=c("forest"))
par(mfrow=c(1,1))
terra::plot(forest, main="Forest cover in 2010", legend=FALSE)
# Representation of species richness restricted to forest cover in 2010
species_richness <- terra::rast("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/species_richness_m.tif")
...
terra::plot(c(species_richness, species_richness_deforest),
            breaks=c(0,5,10,15,20,25,30,35,40,45,50,60,70,80,100,125,150,175,200,
                     300, terra::minmax(species_richness)["max",]), 
...
for(n in 2:npart){
  theta <- terra::rast(paste0("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/RST_centered_theta_", n, ".tif"))
...
terra::writeRaster(coords,
                   filename="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/pca_coords.tif",
                   filetype="GTiff", gdal=c("COMPRESS=LZW", "PREDICTOR=2"), overwrite=TRUE)
...
In the same way as above, we restrict the values obtained for diversity $\beta$ to the forest cover remaining in 2010.
...
# change projection of species turnover raster from lat lon to epsg 32622
species_turnover <- terra::rast("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/species_turnover.tif")
species_turnover_m <- project(species_turnover, forest)
terra::writeRaster(species_turnover_m,
                   filename="~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/species_turnover_m.tif",
                   filetype="GTiff", gdal=c("COMPRESS=LZW", "PREDICTOR=2"), overwrite=TRUE)
# remove species turnover values where there was no forest in 2010
species_turnover <- terra::rast("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/species_turnover_m.tif")
...
# Representation of species turn over restricted to forest cover in 2010
# raster of French Guiana's forest cover in 2010
env.path <- "~/Documents/projet_METRADICA/data/data_raw/environ.tif"
forest <- terra::rast(env.path, lyrs=c("forest"))
# current species turnover
species_turnover_current <- terra::rast("~/Documents/projet_METRADICA/JSDM_French_Guiana/JSDM_French_Guiana_cache/species_turnover_m.tif")
...
(ref:cap-habitat) **_Habitat in French Guiana_** [@Habitats2015].

```{r plot-habitat, echo=FALSE, out.width=800, fig.cap="(ref:cap-habitat)", eval=TRUE}
knitr::include_graphics("JSDM_French_Guiana_files/figure-html/arpercu_habitats_.png")
```

We can see that the estimated map of $\beta$-diversity agrees with the map of French Guiana’s habitat from the book @Habitats2015.     
